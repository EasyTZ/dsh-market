...
- **已安装**：按「用户能拿它怎么办」分三组 —— **桌面自带**（宿主内置、卸不掉的，通常只有市场自己）、**随应用分发但当前没装**（给「装回来」入口，用宿主随包带的副本，离线可用）、**其余全部**（可停用、可卸载）。停用与装卸都要重启内核后生效，面板里会提示。有新版本的会给一个「有更新」徽章和一键更新按钮（保护名单里的包不检查——它们本来就不能经这个市场升级）。
...
**自锁保护**：`PROTECTED_PACKAGES` 里必须包含本插件自己（`readOwnName()`）。踩过一次 —— 市场把自己列成可卸载，点一下就把自己从 profile 里删了，而唯一能装回它的 UI 正是刚被删掉的那个。停用接口同理。
