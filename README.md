...
- **已安装**：按「用户能拿它怎么办」分三组 —— **桌面自带**（宿主内置、卸不掉的；`@deepseek-ai/dsh` 等产品包始终在内，在 dsDesktop 里市场自己也在内，见下面「自锁保护」）、**随应用分发但当前没装**（给「装回来」入口，用宿主随包带的副本，离线可用）、**其余全部**（可停用、可卸载）。停用与装卸都要重启内核后生效，面板里会提示。有新版本的会给一个「有更新」徽章和一键更新按钮（保护名单里的包不检查——它们本来就不能经这个市场升级）。
...
**自锁保护，只在 dsDesktop 里生效**：有 `DSH_DESKTOP_PLUGIN_STATE`（即在 dsDesktop 里）时，保护名单里才会加上本插件自己（`readOwnName()`）。踩过一次 —— 市场把自己列成可卸载，点一下就把自己从 profile 里删了，而唯一能装回它的 UI 正是刚被删掉的那个，在只提供图形界面的 dsDesktop 里这是真死路。停用接口同理，且它本来就整个只在 dsDesktop 里可用。脱离 dsDesktop、自己拿 `dsh plugin add` 装的用户不受这条锁——能敲命令装上它的人，也能敲命令把它装回来，不必替他们把这条路堵死。宿主产品包（`@deepseek-ai/dsh` 等）不受这条环境判断影响，任何时候都保护。
