...
export function normalizeInstalled(profileManifest, readInstalled, protectedNames = []) {
  ...
  for (const [name, spec] of Object.entries(deps)) {
    ...
    items.push({
      ...
      description: typeof manifest?.description === "string" ? manifest.description : "",
      // 已安装卡片要和发现页一样带关键词标签（见 client.js 的 InstalledRow）——
      // 这份数据就在本地包的 package.json 里，读它不用多打一次 npm 请求。
      keywords: Array.isArray(manifest?.keywords) ? manifest.keywords.filter((k) => typeof k === "string") : [],
 ...
 * 把下载量并进条目——**只并，不排，也不擦**。
 *
 * 用在「整份列表已经全局排好、这一页只是要把数字填上」的场合（见 lib/index.js 的
 * handleSearch）。和 applyDownloads 的区别有两条，都是被实际问题逼出来的：
 *
 *   1. 不排序。已经全局排好还再排一次，等于拿这一页重新洗牌，全局顺序当场作废。
 *   2. `downloads` 里没有这个包时，**保留条目自己已有的数字**而不是写成 null。
 *      排序候选池那一步已经把数字并进去了，这一页的补查要是恰好失败（限流、缓存
 *      被挤掉），拿 null 覆盖回去就是「卡片上的下载量凭空消失」。
 *
 * @param {object[]} items 条目
 * @param {Record<string, number>} downloads 包名 → 下载量
 */
export function mergeDownloads(items, downloads) {
  return items.map((item) => ({
    ...item,
    downloads: Object.prototype.hasOwnProperty.call(downloads, item.name)
      ? downloads[item.name]
      : (Number.isFinite(item.downloads) ? item.downloads : null)
  }));
}
