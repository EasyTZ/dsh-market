...
// ---------------------------------------------------------------------------
//
// 这一段原本住在 dsDesktop 的 dsh-plugin-manager 插件里。那个插件删掉之后，它的
// 职责（读桌面清单 / 读写开关状态 / 读激活 overlay）搬到了这里——插件市场既然已经
// 是唯一的插件管理入口，后端也没有理由分在两个包里，隔着一次 HTTP 互相调。
//
// **注意这套语义与 dsDesktop 的 src/shared/plugin-state.js 是同一份规则的两处书写**：
// 那边是外壳进程的「读 + 合并」（生成激活 overlay 时用），这边是「写」。跨仓库、跨
// 进程，没法共用模块。唯一必须同步的是「`enabled` 缺省为 true」这条默认值规则——
// 改它要同时改两边，否则写侧会写出多余的键、或者把用户没动过的项钉死在旧默认上。
