...
import {
  ...
  imageCandidates, installability, isExactVersion, isValidPackageName, normalizeDownloads,
  nextUserState, normalizeInstalled, normalizeManifest, normalizeSearchHits, tailLines
} from "./pure.js";

/**
 * 插件市场（host 半）。
 * 注册 14 条 /api/dsdesktop/market/* 路由。只读的：search / detail / installed / capabilities /
 * image / settings / desktop-plugins / bundled；会写盘的：install / uninstall / settings/save /
 * desktop-plugins/toggle / bundled/install / profile-plugins/toggle。浏览器半 fetch 它们。走 webServer 路由而非 Typert
...
const PROTECTED_PACKAGES = new Set([
  ...
  // **本插件自己**。这是踩出来的：市场把自己也列成了可卸载，点一下就把自己从
  // profile 里删了——而卸载完之后，唯一能把它装回来的 UI 正是刚被删掉的那个，
  // 用户只剩「去终端敲 dsh plugin add」这一条路。这是自我反锁，和 dsh-plugin-manager
  // 里「管理面板自身不可关闭」是同一类保护，只是那边挡的是开关、这边挡的是卸载。
  readOwnName()
]);
...
async function handleSearch(ctx, req, res) {
  ...
  const sort = SORTS.includes(sortParam) ? sortParam : "relevance";
  ...
  try {
    ...
    // 下载量在服务端一次性抓好再回给前端：让浏览器半为每条各打一个请求，既慢
    // 又会在列表滚动时把 api.npmjs.org 打满。抓失败不影响列表本身（见 fetchDownloads）。
    const downloads = await fetchDownloads(items.map((item) => item.name));
    return sendJson(res, 200, { ok: true, data: { total, sort, items: applyDownloads(items, downloads, sort) } });
  } catch (error) {
...
async function fetchDownloads(names) {
  ...
  if (unscoped.length > 0) {
    jobs.push(fetchJson(`${NPM_API}/downloads/point/last-week/${unscoped.join(",")}`).catch(() => null));
  }
  for (const name of scoped) {
    jobs.push(fetchJson(`${NPM_API}/downloads/point/last-week/${name.split("/").map(encodeURIComponent).join("/")}`).catch(() => null));
  }
  const merged = {};
...
async function handleDetail(ctx, req, res) {
  ...
  try {
    ...
    const data = normalizeManifest(manifest, PROTECTED_PACKAGES);
 ...
 */
function handleInstalled(ctx, req, res) {
  if (req.method !== "GET" && req.method !== "HEAD") return sendJson(res, 405, fail("method-not-allowed", "GET only"));
  const dir = profileDir(ctx);
  ...
  const { items } = normalizeInstalled(manifest, readInstalled, PROTECTED_PACKAGES);
  ...
  for (const item of items) {
    ...
    // 必然报错的开关。
    item.canDisable = state !== null && item.entryIds.length > 0 && !PROTECTED_PACKAGES.has(item.name);
...
async function handleInstall(ctx, req, res) {
  ...
  const verdict = installability(manifest, PROTECTED_PACKAGES);
...
async function handleUninstall(ctx, req, res) {
  ...
  const name = body?.name;
  if (!isValidPackageName(name)) return sendJson(res, 200, fail("bad-name", "包名不合法"));
  if (PROTECTED_PACKAGES.has(name)) return sendJson(res, 200, fail("protected", "这是宿主自带的包，不能卸载"));
  const dir = profileDir(ctx);
  if (dir === null) return sendJson(res, 200, fail("no-profile", "定位不到 profile 目录"));
  const manifest = readJsonSafe(join(dir, "package.json"));
...
// ---------------------------------------------------------------------------
//
// 这两条路由原本属于 dsDesktop 的 dsh-plugin-manager 插件。它的 UI 并进本面板之后，
// 后端再单独占一个包、隔着一次 HTTP 被自己调用就没有意义了，于是整个搬过来。
//
// 这一段是**桌面专属**的：靠外壳注入的三个环境变量定位清单、状态文件与激活 overlay。
// 装在裸 dsh 里读不到它们，路由如实返回 missing-env，面板据此不显示「桌面自带」那组
// —— 一个通用插件带一段只有某个宿主用得上的能力，代价就是这点降级判断。

/** 激活 overlay 里 insert 条目的行格式（外壳的 renderActivationPatch 生成）。 */
const INSERT_ID_RE = /^\s*-\s+id:\s*(\S+)\s*$/;

/** 从激活 overlay 里读出「这次启动实际激活了哪些 entryId」；读不到按空集。 */
...
/** 读用户开关状态（只记录偏离清单默认值的项）。读不到按「没有任何覆盖」。 */
 ...
 * 读桌面自带插件的展示信息（描述、版本）。
 *
 * 从外壳给的插件资源目录里读。**打包态**那里是 pack-plugins 摊出来的完整源码，读得到；
 * **开发态**指向仓库的 plugins/ 目录，四个 vendor 插件不在那儿，于是读不到——那种情况
 * 退化成只显示包名，不是错误。原来的 dsh-plugin-manager 是从「兄弟目录」读的（它和那
 * 四个插件一起躺在内核的 node_modules 里），本插件住在 profile 里，没有那层兄弟关系。
 */
function readDesktopPluginMeta(pluginsDir, packageName) {
  const pkg = readJsonSafe(join(pluginsDir, ...packageName.split("/"), "package.json"));
  if (!pkg || typeof pkg !== "object") return { description: null, version: null };
  return {
...
/** GET /desktop-plugins —— 桌面自带插件的清单与当前激活状态。 */
function handleDesktopPlugins(ctx, req, res) {
  if (req.method !== "GET" && req.method !== "HEAD") return sendJson(res, 405, fail("method-not-allowed", "GET only"));
  const env = process.env;
  const pluginsDir = env.DSH_DESKTOP_PLUGINS_DIR;
  if (!pluginsDir) {
    // 不是桌面版。这不是错误，是「这台宿主没有这个概念」，面板据此不显示这一组。
    return sendJson(res, 200, fail("missing-env", "桌面自带插件仅在 dsDesktop 里可用"));
  }
  const manifest = readJsonSafe(join(pluginsDir, "plugins.json"));
  if (!Array.isArray(manifest)) return sendJson(res, 200, fail("manifest-unreadable", "插件清单读不出来"));
  ...
  for (const plugin of manifest) {
    ...
    const meta = readDesktopPluginMeta(pluginsDir, plugin.packageName);
    plugins.push({
      entryId: plugin.entryId,
      packageName: plugin.packageName,
      description: meta.description,
      version: meta.version,
      // 「当前激活」以 overlay 为准：它是外壳那份合并逻辑（清单默认值 × 用户状态）
      // 的输出快照，读它比在这里重新实现一遍可靠。
      active: active.has(plugin.entryId)
    });
  }
  return sendJson(res, 200, { ok: true, data: { plugins, safeMode: env.DSH_DESKTOP_SAFE_MODE === "1" } });
}

/** POST /desktop-plugins/toggle { entryId, enabled } —— 写用户开关，下次启动生效。 */
async function handleDesktopToggle(ctx, req, res) {
  if (req.method !== "POST") return sendJson(res, 405, fail("method-not-allowed", "POST only"));
  const env = process.env;
  const pluginsDir = env.DSH_DESKTOP_PLUGINS_DIR;
  const statePath = env.DSH_DESKTOP_PLUGIN_STATE;
  if (!pluginsDir || !statePath) return sendJson(res, 200, fail("missing-env", "桌面自带插件仅在 dsDesktop 里可用"));
  let body;
  try {
    body = await readJsonBody(req);
  } catch {
  ...
  if (typeof body?.enabled !== "boolean") return sendJson(res, 200, fail("bad-request", "enabled 必须是布尔值"));
  try {
    const state = nextUserState(readUserState(statePath), plugin, body.enabled);
    mkdirSync(dirname(statePath), { recursive: true });
    writeFileSync(statePath, `${JSON.stringify(state, null, 2)}\n`, "utf8");
  } catch (error) {
    return sendJson(res, 200, fail("state-write-failed", `状态写入失败：${error?.message ?? error}`));
...
const ROUTES = [
  ...
  ["/desktop-plugins", handleDesktopPlugins],
  ["/desktop-plugins/toggle", handleDesktopToggle],
...
export const __test__ = { profileDir, PROTECTED_PACKAGES };
