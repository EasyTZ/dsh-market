...
import {
  ...
  extractImages, filterTaggedIndex, imageCandidates, installability, isExactVersion, isUpdateAvailable,
...
const TAGGED_INDEX_TTL_MS = 10 * 60 * 1000;
 ...
 */
async function getTaggedIndex(registryBase) {
  const cached = taggedIndexCache.get(registryBase);
  if (cached && Date.now() - cached.at <= TAGGED_INDEX_TTL_MS) return cached.items;
  ...
  const first = normalizeSearchHits(await fetchJson(firstTarget));
  ...
  if (froms.length > 0) {
    ...
    const pages = await runLimited(froms.map((from) => async () => {
      if (isOfficial) await new Promise((resolve) => setTimeout(resolve, 120));
      const target = `${registryBase}/-/v1/search?text=${encodeURIComponent(searchText)}&size=${NPM_SEARCH_UPSTREAM_MAX}&from=${from}`;
      return normalizeSearchHits(await fetchJson(target)).items;
    }), limit);
  ...
  taggedIndexCache.set(registryBase, { items: tagged, at: Date.now() });
  return tagged;
}
  ...
  try {
    ...
    if (onlyPlugins) {
      if (registryBase === NPM_REGISTRY && qRaw.trim() === "") {
        ...
        try {
          const taggedIndex = await getTaggedIndex(registryBase);
...
async function handleInstall(ctx, req, res) {
  ...
  try {
    ...
    // 就跑什么」是这个市场唯一能给的保证。
    const result = await runDshPlugin(basename(dir.replace(/[\\/]+$/, "")), ["add", `${name}@${version}`], { registryBase });
...
export const __test__ = { profileDir, HOST_PROTECTED_PACKAGES, protectedPackages, disableProtectedPackages };
