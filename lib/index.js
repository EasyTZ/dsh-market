...
import {
  ...
  declaredBundlePatch, extractImages, filterTaggedIndex, imageCandidates, installability, isExactVersion, isUpdateAvailable, mergeDownloads,
...
/**
 * 插件市场（host 半）。
 * 注册 13 条 /api/dsdesktop/market/* 路由。只读的：search / detail / installed / updates /
 * capabilities / image / settings / bundled；会写盘的：install / uninstall / settings/save /
 * bundled/install / profile-plugins/toggle。浏览器半 fetch 它们。走 webServer 路由而非 Typert
...
// TTL 从 10 分钟放宽到 6 小时，跟 tagged 索引、下载量那两份缓存对齐。
//
// 10 分钟是「详情页偶尔展开一次」那个年代定的。缩略图上来之后这个数字的含义变了：
// 面板每开一次就要为**首屏每一张卡片**问一遍图，10 分钟一到，同一批包又得各抓一遍
// 416KB 的 packument——用户的体感就是「每次打开都要加载半天」。
//
// 放宽是安全的：这里存的是「README 里第几张图的地址」，它只在作者发新版并改动
// README 时才会变，而那是以天计的事。最坏后果是详情里显示的是上一版的截图。
const README_TTL_MS = 6 * 60 * 60 * 1000;
// 上限从 40 提到 200：发现页的卡片现在会为**每个可见的包**要一张缩略图（见
// handlePreview），一屏就是二十几个，40 条一滚就被自己挤光，滚回去又得重抓一遍
// 416KB 的 packument。缓存里存的只是几条 URL 字符串（README 本身不留），200 条的
// 内存代价可以忽略。
const README_CACHE_MAX = 200;
/** 同一个包正在飞的那次抓取，见 imagesFor 的注释。 */
...
function cacheGet(name) {
  const hit = cacheEntry(name);
  return hit === null ? null : hit.images;
}

/**
 * 这个包的 latest 版本号（抓 README 那次顺手从 packument 的 dist-tags 里拿的）。
 *
 * 只有一个用处，但那个用处很要命：**卡片缩略图的 jsDelivr 候选需要版本号**。
 * README 里写相对路径的图（作者最该用的那种写法，见 imageCandidates）会被还原成
 * 两个候选——`cdn.jsdelivr.net/npm/<包>@<版本>/<路径>` 和 `raw.githubusercontent.com`。
 * 没有版本号，jsDelivr 那条**根本不会生成**，于是只剩 raw.githubusercontent.com，
 * 而它在国内网络多半是直接超时：详情页有图（那条路走 /latest，手里有版本号），
 * 同一个包的卡片缩略图却是空的。
 *
 * 单独为它打一次 `/latest` 是不必要的——抓 README 的那份全量 packument 里就带着
 * dist-tags，顺手存下来即可，零额外请求。
 */
function cacheGetVersion(name) {
  const hit = cacheEntry(name);
  return hit === null || typeof hit.version !== "string" ? null : hit.version;
}

function cacheSet(name, images, version) {
  ...
  readmeCache.set(name, { images, version: typeof version === "string" ? version : null, at: Date.now() });
  scheduleImagesIndexSave();
}
...
// ---------------------------------------------------------------------------
//
// **为什么内存缓存不够。** 上面那份 readmeCache 挂在内核进程上，内核一重启就空了
// ——而装/卸/更新任何一个插件都要重启内核。于是「装个插件再回来看看」这个再普通
// 不过的动作，代价是首屏二十几个包各重抓一次 416KB 的 packument。
//
// 落盘之后这件事变成一台机器上**每个包只发生一次**（TTL 内）。存的东西和内存里
// 那份一模一样：几条 URL 字符串 + 抓取时间，README 正文从来不留，200 条也就几十 KB。
//
// 和 tagged 索引、下载量索引一样，这是**纯缓存**：文件读不出来、格式不对、写不进去，
// 最坏后果都只是重抓一次，绝不能因此让任何一条路由报错。
let imagesIndexLoaded = false;
let imagesIndexSaveTimer = null;
/**
 * 落盘用的 ctx。cacheSet 在 imagesFor 里被调到，那条路上没有 ctx——而每条会用到
 * 图片的路由（detail / preview / image）进来时都会先 loadImagesIndex(ctx)，顺手把它
 * 记在这里。拿不到（还没有任何路由被调过）就干脆不落盘。
 */
let imagesIndexCtx = null;

function imagesIndexFile(ctx) {
  const dir = profileDir(ctx);
  return dir === null ? null : join(dir, "dsh-market-images.json");
}

/**
 * 把磁盘上那份图片清单读进内存缓存。正常情况下**每个进程只做一次**。
 *
 * 「内存缓存空了」也重读一次：那要么是换源时被清掉了（那时磁盘那份也一并清空了，
 * 读回来是空的，符合预期），要么是内核刚起来还没读过。判据用 readmeCache.size 而不是
 * 只看那个布尔，跟 getTaggedIndex 的「内存没有就问磁盘」是同一套。
 */
function loadImagesIndex(ctx) {
  imagesIndexCtx = ctx;
  if (imagesIndexLoaded && readmeCache.size > 0) return;
  imagesIndexLoaded = true;
  const file = imagesIndexFile(ctx);
  if (file === null) return;
  const stored = readJsonSafe(file);
  if (!stored || typeof stored !== "object") return;
  for (const [name, entry] of Object.entries(stored)) {
    if (readmeCache.size >= README_CACHE_MAX) break;
    if (!entry || typeof entry.at !== "number" || !Array.isArray(entry.images)) continue;
    // 时间原样带回来：过期与否由 cacheGet 按 README_TTL_MS 判，重启不该让一份
    // 刚抓的缓存重新变「新」，也不该让一份很旧的缓存续命。
    readmeCache.set(name, {
      images: entry.images,
...
/** 攒多久写一次盘，见 scheduleImagesIndexSave。 */
const IMAGES_INDEX_SAVE_DELAY_MS = 5000;

/**
 * 延迟落盘。一屏卡片会连着调二十几次 cacheSet，每次都写一遍文件是纯浪费——攒一会儿
 * 整份写出去。定时器 unref：它不该让内核多活哪怕一秒。
 */
function scheduleImagesIndexSave() {
  if (imagesIndexCtx === null || imagesIndexSaveTimer !== null) return;
  imagesIndexSaveTimer = setTimeout(() => {
    imagesIndexSaveTimer = null;
    saveImagesIndex(imagesIndexCtx);
  }, IMAGES_INDEX_SAVE_DELAY_MS);
  if (typeof imagesIndexSaveTimer.unref === "function") imagesIndexSaveTimer.unref();
}

function saveImagesIndex(ctx) {
  const file = ctx === null ? null : imagesIndexFile(ctx);
  if (file === null) return;
  try {
    const stored = {};
    for (const [name, hit] of readmeCache) stored[name] = { at: hit.at, version: hit.version ?? null, images: hit.images };
...
// TTL 从 30 分钟放宽到 6 小时，跟 tagged 索引的新鲜度对齐：周/月下载量本来就是
// 按天变的数字，半小时就过期只会让排序候选池（SORT_POOL，上千个包）反复重抓。
const DOWNLOADS_TTL_MS = 6 * 60 * 60 * 1000;
// 池子按周/月各存一份，再留出翻页时零散查到的包。条目只是「字符串 → 数字」，
// 一万条的内存代价可以忽略。
const DOWNLOADS_CACHE_MAX = 10000;
/** npm 批量下载量接口一次最多 128 个包，留点余量。 */
const DOWNLOADS_BULK_MAX = 100;
/** 磁盘上那份下载量索引有没有读进来过，见 loadDownloadsIndex。 */
...
function downloadsCacheSet(period, name, value, at = Date.now()) {
  ...
  downloadsCache.set(downloadsCacheKey(period, name), { value, at });
}
...
`, "utf8");
  ...
  // 清掉缓存让它们重来一遍。磁盘上那份也要跟着清空，否则下次内核重启又把旧结论
  // 读回来——用户切镜像正是为了甩掉那批结论。
  readmeCache.clear();
  saveImagesIndex(ctx);
...
async function imagesFor(name, slug, registryBase) {
  ...
  // 同名的并发请求合流成一次。发现页一屏二十几张卡片同时要缩略图（handlePreview），
  // 用户又可能在图还没回来时点开同一个包的详情——没有这一层，同一个 416KB 的
  // packument 会被同时抓好几遍，缓存要等第一个回来才开始起作用。
  const inflight = readmeInflight.get(name);
  if (inflight !== undefined) return inflight;
  const task = (async () => {
    let readme = null;
    // 版本号是**顺手**取的：同一份 packument 里就带着 dist-tags，不多打一次请求。
    // 它只服务缩略图那条路上的 jsDelivr 候选，见 cacheGetVersion。
    let version = null;
    try {
      const packument = await fetchJson(`${registryBase}/${packageUrlPath(name)}`);
    ...
    cacheSet(name, images, version);
    return images;
  })().finally(() => { readmeInflight.delete(name); });
  readmeInflight.set(name, task);
  return task;
}
 ...
 * 「按下载量排序」的候选池大小。
 *
 * **为什么需要一个池子。** npm 的 `/-/v1/search` 根本不提供按下载量排序，下载量得
 * 走另一个接口一个个查。之前的做法是「先按 npm 的相关度序切出这一页的 24 条，再
 * 查这 24 条的下载量、把这 24 条排一下」——于是每一页各自排各自的，第一页排到 78
 * 结尾，第二页从 2745 重新开始，第三页又冒出个 13857。用户看到的就是「排序不对」。
 *
 * 要全局正确就得先拿到**全部**候选包的下载量。全量索引现在是 3500+ 个包，其中近
 * 三成是 scoped，而 npm 的批量接口不支持 scoped——全量意味着每次刷新上千次请求
 * （开着国内镜像时因为完全不支持批量，是 3500 次）。所以取一个池子：只对索引里
 * **靠前的 1000 个**取下载量并全局排序，池子外的按原顺序接在后面。
 *
 * 1000 这个数不是拍的：索引本身是 npm 的相关度序，而相关度里下载量占很大权重，
 * 真正的下载量前几十名不可能掉出前 1000。按一页 24 条算，池子覆盖前 41 页——
 * 「按下载量排序」这个动作的用户不会翻到那儿去。
 */
const SORT_POOL = 1000;

/** 下载量索引落盘的位置。跟 tagged 索引同一个目录，理由见 settingsFile。 */
function downloadsIndexFile(ctx) {
  const dir = profileDir(ctx);
  return dir === null ? null : join(dir, "dsh-market-downloads.json");
}

/**
 * 把磁盘上那份下载量索引读进内存缓存。**每个进程只做一次。**
 *
 * 没有这一步，池子在每次内核重启后都是空的：第一次搜索排不出全局顺序（只能退回
 * 按页排），要等后台把上千个包填完才恢复。落盘之后这件事一台机器上只发生一次。
 *
 * 和 tagged 索引一样，这是**纯缓存**：读不出来、格式不对，最坏后果是重填一次。
 */
function loadDownloadsIndex(ctx) {
  if (downloadsIndexLoaded) return;
  downloadsIndexLoaded = true;
  const file = downloadsIndexFile(ctx);
  if (file === null) return;
  const stored = readJsonSafe(file);
  if (!stored || typeof stored !== "object") return;
  for (const [period, entry] of Object.entries(stored)) {
    if (!entry || typeof entry.at !== "number" || !entry.values || typeof entry.values !== "object") continue;
    // 落盘时间当成每一条的时间：这份表是整批填的，逐条记时间只是把文件撑大。
    for (const [name, value] of Object.entries(entry.values)) {
      if (Number.isFinite(value)) downloadsCacheSet(period, name, value, entry.at);
    }
  }
}

/** 把内存里某个周期的下载量写回磁盘，保留另一个周期那份。写不进去只是下次要重填。 */
function saveDownloadsIndex(ctx, period) {
  const file = downloadsIndexFile(ctx);
  if (file === null) return;
  try {
    const values = {};
    // 前缀用 downloadsCacheKey 现算，别在这儿再写一遍分隔符——两处对不上时
    // 表现是「落盘的文件永远是空的」，而那是个不会报错的坏法。
    const prefix = downloadsCacheKey(period, "");
    for (const [key, hit] of downloadsCache) {
      if (key.startsWith(prefix)) values[key.slice(prefix.length)] = hit.value;
    }
    const stored = readJsonSafe(file) ?? {};
    stored[period] = { at: Date.now(), values };
    writeFileSync(file, JSON.stringify(stored), "utf8");
  } catch {
    // 忽略：这只是缓存。
  }
...
/** 正在后台填的周期，同一个周期同时只跑一轮。 */
const downloadsFillRunning = new Set();

/**
 * 取候选池的下载量，**绝不阻塞这次搜索**。
 *
 * 冷启动时池子里上千个包一个都没有，真去等它填完是几分钟——那期间用户对着一个转圈
 * 的列表。所以这里只返回「现在已经知道的」，缺的丢给后台慢慢填（填完落盘，见
 * loadDownloadsIndex：这件事一台机器上只发生一次）。
 *
 * 池子还没填够时**退回按页排**，而不是拿半张表去做全局排序：半张表排出来的顺序
 * 是「知道下载量的排前面、不知道的沉底」，那比按页排更莫名其妙——用户会看到几个
 * 冷门包排在热门包前面，且刷新一次顺序就变一次。宁可维持旧行为，也不要一个会抖的
 * 新行为。
 *
 * @returns {{ values: Record<string, number>, ready: boolean }}
 */
function poolDownloads(ctx, names, period, registryBase) {
  loadDownloadsIndex(ctx);
  const values = {};
  const missing = [];
  for (const name of names) {
    const cached = downloadsCacheGet(period, name);
    if (cached === null) missing.push(name);
    else values[name] = cached.value;
  }
  if (missing.length > 0 && !downloadsFillRunning.has(period)) {
    downloadsFillRunning.add(period);
    void (async () => {
      try {
        await fetchDownloads(missing, period, registryBase);
        saveDownloadsIndex(ctx, period);
      } catch {
        // 填失败就下次再填：这一路上没有任何东西在等它。
      } finally {
        downloadsFillRunning.delete(period);
      }
    })();
  }
  // 留 5% 的余量：总有几个包的下载量是真查不到的（刚发布、被 unpublish），
  // 要求 100% 齐会让全局排序永远开不了。
  return { values, ready: missing.length <= Math.ceil(names.length * 0.05) };
}

/**
 * 把整份候选列表按当前排序**全局**排好，再交给分页去切。
 *
 * 「最近更新」是免费的：日期本来就在索引里，排完再切片就行，一次额外请求都不用打。
 * 「下载量」要看候选池填好没有（见 poolDownloads）。
 *
 * @returns {{ items: object[], globallySorted: boolean }}
 */
function orderBySort(ctx, items, sort, registryBase) {
  if (sort === "updated") return { items: applyDownloads(items, {}, "updated"), globallySorted: true };
  const period = sort === "downloads-month" ? "last-month" : "last-week";
  const pool = items.slice(0, SORT_POOL);
  const { values, ready } = poolDownloads(ctx, pool.map((item) => item.name), period, registryBase);
  if (!ready) return { items, globallySorted: false };
  // 池子外的原样接在后面：它们的下载量没查过，查了也排不进前 1000。
  return { items: [...applyDownloads(pool, values, "downloads"), ...items.slice(SORT_POOL)], globallySorted: true };
}
...
async function handleSearch(ctx, req, res) {
  ...
  try {
    let total, items;
    // 排序是**先排后切**还是**切了再排**：前者全局正确，后者每页各排各的（那正是
    // 「按下载量排序不对」的成因，见 SORT_POOL）。只有走全量索引这条路才排得动
    // ——手上得先有整份候选列表。
    let globallySorted = false;
    if (onlyPlugins) {
      try {
        // 官方源空查询以前走 `keywords:` 限定、直接把分页转发给 npm，省下全量索引
        // 那 15 页请求。但那条路手上永远只有当前这一页，排序不可能全局正确，所以
        // 现在两种查询都走索引——它缓存 6 小时、过期也是先返回旧的再后台刷新
        // （见 getTaggedIndex），那 15 页请求不在用户等待的路径上。`keywords:` 那条
        // 老路留作下面 catch 里的降级。
        //
        // refresh=1 只有顶部那个刷新按钮会带：索引缓存 6 小时，用户得有一个
        ...
        const ordered = orderBySort(ctx, filtered, sort, registryBase);
        globallySorted = ordered.globallySorted;
        total = ordered.items.length;
        items = ordered.items.slice(from, from + size);
      } catch (error) {
        ...
        const text = registryBase === NPM_REGISTRY && qRaw.trim() === ""
          ? `keywords:${DEFAULT_KEYWORD}`
          : buildSearchText(qRaw);
        const target = `${registryBase}/-/v1/search?text=${encodeURIComponent(text)}&size=${NPM_SEARCH_UPSTREAM_MAX}&from=0`;
      ...
      // 「搜整个 npm」这一路没有全量候选列表可言（结果可能几万条，而且每换一个
      // 查询词就是一份新的），只能拿 npm 的相关度序分页、再把这一页排一下。
    ...
    // 已经全局排好的就**只把数字并进去，不再排**：再排一次是拿这一页重新洗牌，
    // 全局顺序当场作废（池子外那些没有下载量的条目会被一路顶到这一页的最后）。
    // 没能全局排的（索引挂了退化成一页、或者「搜整个 npm」）才退回按页排——
    // 那是旧行为，不完美，但比不排强。
    //
    // 周/月该抓哪个区间是上面那行的事，applyDownloads / mergeDownloads 只管拿到的
    // 数字怎么处理。
    const items2 = globallySorted
      ? mergeDownloads(items, downloads)
      : applyDownloads(items, downloads, sort === "updated" ? "updated" : "downloads");
...
async function fetchDownloads(names, period = "last-week", registryBase = NPM_REGISTRY) {
  ...
  if (registryBase === NPM_REGISTRY) {
    ...
    // **批量查要分段。** npm 的批量接口一次最多 128 个包，而这个函数现在也用来
    // 填排序候选池（上千个名字，见 SORT_POOL）——把上千个名字拼进一条 URL，
    // 要么被网关按 URL 长度拒掉，要么被接口自己拒掉，然后**整批**下载量都没了。
    for (let at = 0; at < unscoped.length; at += DOWNLOADS_BULK_MAX) {
      const chunk = unscoped.slice(at, at + DOWNLOADS_BULK_MAX);
      tasks.push(async () => {
        const payload = await fetchJson(`${NPM_API}/downloads/point/${period}/${chunk.join(",")}`).catch(() => null);
  ...
  // 官方源以前走 `Promise.all` 全并行，理由是「一轮超时会被并发器放大成好几轮」。
  // 那在「一页 24 个名字」的年代成立；排序候选池上来之后，同一次调用可能有几百个
  // scoped 单查任务，全并行就是一口气对着 api.npmjs.org 开几百条连接——先撞的是
  // 限流，然后是所有请求一起变慢。两边统一走并发器，官方源给宽一点：它的任务里
  // 大头是批量查（几百个名字才几条请求），剩下的 scoped 单查响应也快。
  const results = await runLimited(tasks, registryBase === NPM_REGISTRY ? 8 : 6);
...
/**
 * GET /preview?name= —— 发现页卡片的缩略图，只要**第一张**。
 *
 * 为什么不让 /detail 顺手带上：详情是「点开一条才打」的请求，缩略图是「卡片进
 * 视野就要」的请求，二者的调用量差一个数量级。分开之后卡片这条路只回一个字符串，
 * 而且和详情共用同一份 README 缓存与并发合流（imagesFor）——用户先看到缩略图、
 * 再点开详情时，详情那一步不会再抓一次 packument。
 *
 * 取不到就 `src: null`（网络不通、README 没图、包被下架都归到这一类）。这条路由
 * **永远不报错**：卡片上少一张图是可以接受的降级，弹一个错误提示不是。
 */
async function handlePreview(ctx, req, res) {
  ...
  loadImagesIndex(ctx);
  const { registryBase } = readSettings(ctx);
  // github slug 从搜索结果里带过来（客户端有，服务端这条路上没有）。给不出也无所谓：
  // 只影响 README 里那些相对路径的图能不能被还原成 GitHub raw 地址。
  // 形状卡死成 `owner/repo`：它会被拼进 raw.githubusercontent.com 的路径。主机白名单
  // 已经挡住了「指向别处」，这一层挡的是拼出奇怪路径（`../`、查询串）那类花活。
  const slugParam = url.searchParams.get("slug");
  const slug = typeof slugParam === "string" && /^[\w.-]{1,39}\/[\w.-]{1,100}$/.test(slugParam) ? slugParam : null;
  try {
    const images = await imagesFor(name, slug, registryBase);
    if (images.length === 0) return sendJson(res, 200, { ok: true, data: { src: null } });
    // 和 /detail 用同一个代理地址形状：浏览器半永远不直连 README 里写的主机。
    //
    // 版本号带上（上面那次 packument 顺手拿的，见 cacheGetVersion）。**这一段不能
    // 留空**：/image 拿它拼 jsDelivr 候选，缺了就只剩 raw.githubusercontent.com 一条
    // 路——详情页有图、卡片缩略图却是空白，正是这么来的。真拿不到就留空，退回旧行为。
    return sendJson(res, 200, { ok: true, data: {
      src: `${ROUTE_PREFIX}/image?name=${encodeURIComponent(name)}&i=0&v=${encodeURIComponent(cacheGetVersion(name) ?? "")}`,
      alt: images[0].alt ?? ""
    } });
  } catch {
    return sendJson(res, 200, { ok: true, data: { src: null } });
  }
}
...
async function handleInstalled(ctx, req, res) {
  if (req.method !== "GET" && req.method !== "HEAD") return sendJson(res, 405, fail("method-not-allowed", "GET only"));
  const result = await collectInstalled(ctx);
  // 面板每次打开都会走这里，而它算出来的正是角标要的那份信息——顺手喂给缓存：
  // 用户升完插件把面板一关，侧边栏那个叹号立刻就是对的，不用等下一轮轮询。
  if (result.ok) updatesCache = { at: Date.now(), names: updatableNames(result.data.items) };
  return sendJson(res, 200, result);
}

/**
 * /installed 的正文。抽成函数是因为 /updates 要的是同一份数据，只是最后只取一个
 * 数字——两边各读一遍 profile、各判一次「有没有更新」迟早会给出不一致的答案。
 */
async function collectInstalled(ctx) {
  const dir = profileDir(ctx);
  if (dir === null) return fail("no-profile", "定位不到 profile 目录");
  const manifest = readJsonSafe(join(dir, "package.json"));
  if (manifest === null) return fail("no-profile", "读不到 profile 的 package.json");
...
/** 已安装列表里「有新版本」的那些包名。侧边栏角标只需要这一个数。 */
function updatableNames(items) {
  return items.filter((item) => item.updateAvailable).map((item) => item.name);
}

/**
 * /updates 的缓存。**必须有**：这条路由是给侧边栏角标轮询用的，而算一次要给每个
 * 可升级的包打一发 npm 请求。多开一个窗口就是多一轮轮询——「有没有更新」这件小事
 * 不该把 registry 打出 429 来（跟 taggedIndexCache 是同一个理由）。
 */
let updatesCache = null;
const UPDATES_FRESH_MS = 10 * 60 * 1000;

/**
 * GET /updates —— 侧边栏那个小叹号要的全部信息：有几个已安装插件能升级。
 *
 * 单独开一条而不是让角标去调 /installed：角标要的是一个数字，/installed 的响应却
 * 带着每个包的停用状态、entry id、bundled 标记等一大堆用不上的东西，而且没有缓存。
 * `?force=1` 跳过缓存。
 */
async function handleUpdates(ctx, req, res) {
  if (req.method !== "GET" && req.method !== "HEAD") return sendJson(res, 405, fail("method-not-allowed", "GET only"));
  const url = new URL(req.url, "http://localhost");
  const force = url.searchParams.get("force") === "1";
  if (!force && updatesCache !== null && Date.now() - updatesCache.at < UPDATES_FRESH_MS) {
    return sendJson(res, 200, { ok: true, data: { count: updatesCache.names.length, names: updatesCache.names } });
  }
  const result = await collectInstalled(ctx);
  // 读不到 profile 就如实报错，**不要**回一个 count:0：那会把「查不了」画成
  // 「没有更新」，用户永远等不到那个叹号，也不知道为什么等不到。
  if (!result.ok) return sendJson(res, 200, result);
  const names = updatableNames(result.data.items);
  updatesCache = { at: Date.now(), names };
  return sendJson(res, 200, { ok: true, data: { count: names.length, names } });
}
...
export const __test__ = {
  profileDir, dshBinPath, missingBundlePatch, handlePreview, readmeCache,
  loadImagesIndex, saveImagesIndex,
  handleSearch, orderBySort, downloadsCache, SORT_POOL,
