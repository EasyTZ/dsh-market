window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".dsmkCardDiscover{height:320px;padding-bottom:74px;overflow:hidden}",
			// 简介是这张卡片里唯一可压缩的部分：高度钉死之后，徽章多一行、图有没有，
			// 都由它吸收（min-height:0 才让 flex 收缩真的生效）。它自己有 line-clamp
			// 和 overflow:hidden，收缩的结果是少显示一两行字，不会溢出。
			// 不给 flex-grow：短简介后面留白，跟以前一样，关键词标签仍紧跟着正文。
			".dsmkCardDiscover .dsmkCardDesc{flex:0 1 auto;min-height:0}",
		...
		function DiscoverRow({ t, item, expanded, onToggle, installedNames, downloadsLabel, showTagged, action, canInstall, busy, onInstall }) {
			...
			// 缩略图：卡片滚进视野才去要（代价见 loadPreview 的注释）。三个态必须分开
			// ——`undefined`（THUMB_PENDING）＝还没有答案，`null`＝问过了、没有图，
			// 字符串＝地址。混为一谈的话，没有图的包每次重渲染都会再排一次队。
			//
