window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// 排序：原生 select，给它一个跟其它输入控件一致的边框/底色——之前是完全
			// 透明、无边框，看着不像个可交互控件，容易被当成普通文字忽略过去。
			".dsmkSortSelect{height:24px;padding:0 6px;border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));border-radius:7px;background:var(--dsw-specific-input-major,#2c2c2e);color:var(--dsw-alias-label-secondary,#cfd3d6);font-size:11.5px;font-family:inherit;cursor:pointer}",
			".dsmkSortSelect:hover{border-color:var(--dsw-alias-border-l2,rgba(255,255,255,.16))}",
			...
			".dsmkCard{position:relative;display:flex;flex-direction:column;gap:6px;min-width:0;padding:12px;border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));border-radius:12px;background:var(--dsw-alias-bg-layer-1,#151517);color:inherit;font-family:inherit;font-size:13px;text-align:left;transition:background .15s ease,border-color .15s ease}",
			"button.dsmkCard{cursor:pointer}",
			".dsmkCardStatic{cursor:default}",
			"button.dsmkCard:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.06))}",
			...
			".dsmkCardFooter{display:flex;justify-content:flex-end;gap:8px;margin-top:2px}",
		...
		/** 发现 tab 的一张卡片：点开进入详情态（见 MarketPanelBody 里的 discoverContent）。 */
		function DiscoverRow({ t, item, expanded, onToggle, installedNames, downloadsLabel, showTagged }) {
			...
			return react_jsx_runtime.jsxs("button", {
				type: "button",
				className: "dsmkCard" + (expanded ? " dsmkCardSelected" : ""),
				...
				children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsmkCardMeta", children: [
						// 下载量排在最前：它是这张卡片里信息量最大的一个数字（有多少人在用）。
		...
		function MarketPanelBody({ t, open, onClose }) {
			...
			react.useEffect(() => { setExpanded(null); }, [tab, debounced, onlyDsh, sort]);
					...
					return react_jsx_runtime.jsxs("div", { className: "dsmkDetailView", children: [
						...
						react_jsx_runtime.jsxs("div", { className: "dsmkDetailCard", children: [
							...
							react_jsx_runtime.jsx(DiscoverDetail, {
								...
								installed: installedNames.has(expanded), action, busy, onInstall: runInstall,
			...
			const installedContent = () => {
				...
				return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsmkGroup", children: [
						...
						// 停用/启用是改 patch 文件，要下次内核启动才读 —— 光说「重启后生效」
						// 等于把活儿丢回给用户，而重启入口就在手边（外壳的 preload 桥）。
						// 和安装/卸载结果条用同一套：有桥给按钮，没桥（别人的 dsh）给一句话。
						togglePending > 0 ? react_jsx_runtime.jsxs("div", { className: "dsmkPending", children: [
							react_jsx_runtime.jsx("span", { children: fmt(t("market.toggle.pending"), { n: togglePending }) }),
							restartAvailable()
								? react_jsx_runtime.jsx("button", { type: "button", className: "dsmkGhostBtn", onClick: () => window.desktop.restartApp(), children: t("market.detail.restart") })
								: null
						] }) : null,
			...
			return react_jsx_runtime.jsxs("div", { className: "dsmkPanel" + (open ? " dsmkOpen" : ""), children: [
				...
				tab === "discover" ? react_jsx_runtime.jsxs("div", { className: "dsmkSearch", children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsmkSearchMeta", children: [
						...
						react_jsx_runtime.jsxs("div", { className: "dsmkSearchControls", children: [
							react_jsx_runtime.jsxs("label", { className: "dsmkSortField", children: [
				...
				react_jsx_runtime.jsx("div", { className: "dsmkBody", onScroll: onBodyScroll, children: tab === "discover" ? discoverContent() : installedContent() }),
