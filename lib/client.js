window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"market.group.desktopHint": "拷进内核分发的插件，可开关但不能卸载",
			"market.desktop.selfLocked": "自身不可关闭",
			"market.desktop.switchLabel": "开关 {name}",
			"market.desktop.pending": "有 {n} 项开关改动，重启后生效",
			"market.desktop.safeMode": "安全模式：本次启动只加载了恢复用的面板，其余插件一律跳过。把可疑插件关掉后重启即可恢复正常启动。",
			"market.desktop.toggleFailed": "开关失败：{msg}"
		};
		const en = {
			...
			"market.badge.protected": "Built-in",
			...
			"market.footer.profile": "profile: {name}",
			"market.group.desktop": "Bundled into the kernel",
			...
			"market.group.desktopHint": "Copied into the kernel — can be toggled, not uninstalled",
			"market.desktop.selfLocked": "Always on (recovery panel)",
			"market.desktop.switchLabel": "Toggle {name}",
			"market.desktop.pending": "{n} toggle(s) pending — restart to apply",
			"market.desktop.safeMode": "Safe mode: only the recovery panel was loaded this time; all other plugins were skipped. Turn off the suspect plugin, then restart.",
			"market.desktop.toggleFailed": "Toggle failed: {msg}"
		};
		...
		const css = [
			...
			".dsmkDangerBtn{height:26px;padding:0 10px;border:1px solid transparent;border-radius:7px;background:transparent;color:var(--dsw-alias-label-tertiary,#8b949e);font-size:12px;font-family:inherit;cursor:pointer;transition:background .15s ease,color .15s ease}",
			".dsmkDangerBtn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08));color:var(--dsw-alias-label-primary,#f9fafb)}",
			".dsmkDangerBtnArmed{background:var(--dsw-alias-state-error-primary,#f0617a);color:var(--dsw-alias-label-primary-inverted,#fff)}",
		 ...
		 * 桌面自带插件的一行：只能开关，不能卸载。
		 *
		 * 「装」和「激活」在桌面版是分开的两件事——被关掉的插件照样装在内核里，只是激活
		 * overlay 里不生成它的条目。所以这里没有卸载按钮不是偷懒：那个包本来就不该由用户
		 * 删，它跟着内核走，删了下次内核热更新又会装回来。
		 *
		 * `active` 是**当前这次启动实际激活的**状态（读的是激活 overlay），不是用户刚拨的
		 * 意愿。拨完开关到重启之间两者会不一致，所以这里显示的是 pending ?? active，
		 * 并在上面挂一句「重启后生效」——否则用户拨完看到开关弹回去，会以为没生效。
		 */
		function DesktopRow({ t, item, desired, onToggle, disabled }) {
			return react_jsx_runtime.jsx("div", { className: "dsmkRow", children:
				react_jsx_runtime.jsxs("div", { className: "dsmkRowHead", style: { cursor: "default" }, children: [
					react_jsx_runtime.jsxs("div", { className: "dsmkRowMain", children: [
						react_jsx_runtime.jsxs("div", { className: "dsmkRowTitle", children: [
							react_jsx_runtime.jsx("span", { className: "dsmkRowName", children: item.packageName }),
							item.version ? react_jsx_runtime.jsx("span", { className: "dsmkRowVersion", children: item.version }) : null,
							item.self ? react_jsx_runtime.jsx(Badge, { kind: "Muted", children: t("market.desktop.selfLocked") }) : null
						] }),
						item.description ? react_jsx_runtime.jsx("div", { className: "dsmkRowDesc", children: item.description }) : null
					] }),
					react_jsx_runtime.jsx("div", { className: "dsmkRowActions", children:
						react_jsx_runtime.jsxs("label", {
							...
							title: fmt(t("market.desktop.switchLabel"), { name: item.packageName }),
							children: [
								react_jsx_runtime.jsx("input", {
									type: "checkbox",
									checked: desired,
									disabled: Boolean(item.self) || disabled,
									"aria-label": fmt(t("market.desktop.switchLabel"), { name: item.packageName }),
									onChange: (e) => onToggle(item.entryId, e.target.checked)
								}),
		...
		function InstalledRow({ t, item, action, busy, onUninstall, canInstall }) {
			...
			return react_jsx_runtime.jsxs("div", { className: "dsmkRow", children: [
				react_jsx_runtime.jsxs("div", { className: "dsmkRowHead", style: { cursor: "default" }, children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsmkRowActions", children: [
						// 停用开关排在卸载左边：它是可逆的、低风险的那个动作，用户会更常用它。
						// 卸载是破坏性的，放在最右而且要点两下。
						...
						item.removable ? react_jsx_runtime.jsx("button", {
							type: "button",
		...
		function MarketPanelBody({ t, open, onClose }) {
			...
			// 桌面自带的那批插件。这两条路由就在本插件里（原先属于 dsh-plugin-manager，
			// 那个包已经合并进来了）。裸 dsh 里它们返回 missing-env —— 那不是错误，是
			// 「这个宿主没有『桌面自带插件』这个概念」，此时整组不显示，面板照常可用。
			const [desktop, setDesktop] = react.useState({ available: false, plugins: [], safeMode: false });
			const [pending, setPending] = react.useState({});
			const [toggleError, setToggleError] = react.useState(null);
			react.useEffect(() => {
				if (!open) return undefined;
				let alive = true;
				getJson("/api/dsdesktop/market/desktop-plugins").then((result) => {
					if (!alive) return;
					if (result && result.ok) setDesktop({ available: true, plugins: result.data.plugins, safeMode: Boolean(result.data.safeMode) });
					else setDesktop({ available: false, plugins: [], safeMode: false });
				}).catch(() => {
					if (alive) setDesktop({ available: false, plugins: [], safeMode: false });
				});
				return () => { alive = false; };
			}, [open, rescan]);

			// 拨完开关到重启之间，「意愿」和「实际激活」是两回事：列表接口只报得出后者，
			// 所以前者记在这儿。等于 0 项时说明用户把改动又拨回去了，不该再提示重启。
			const pendingCount = react.useMemo(() => desktop.plugins.filter(
				(p) => Object.prototype.hasOwnProperty.call(pending, p.entryId) && pending[p.entryId] !== p.active
			).length, [desktop.plugins, pending]);

			const onToggleDesktop = react.useCallback(async (entryId, enabled) => {
				setToggleError(null);
				// 乐观更新：开关是个高频、低风险的交互，等一个往返再动会显得很黏。
				setPending((prev) => ({ ...prev, [entryId]: enabled }));
				try {
					const result = await postJson("/api/dsdesktop/market/desktop-plugins/toggle", { entryId, enabled });
					if (!result || !result.ok) {
						// 失败要把开关拨回去，不能让 UI 停在一个服务端并不认可的状态上。
						setPending((prev) => {
							const next = { ...prev };
							delete next[entryId];
							return next;
						});
						setToggleError(fmt(t("market.desktop.toggleFailed"), { msg: (result && result.error && result.error.message) || "" }));
					}
				} catch (error) {
					setPending((prev) => {
						const next = { ...prev };
					...
					setToggleError(fmt(t("market.desktop.toggleFailed"), { msg: String(error && error.message ? error.message : error) }));
				}
			}, [t]);
			...
			const installedContent = () => {
				...
				return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
					// 安全模式的说明要顶在最上面：那一次启动里所有插件都显示成「未激活」，
					// 不解释一句用户只会更慌（以为插件全丢了）。
					desktop.safeMode ? react_jsx_runtime.jsx("div", { className: "dsmkSafeMode", children: t("market.desktop.safeMode") }) : null,

					// 迁移之后这一组**恒为空**（A2 清单已清空，五个插件全在 profile 层）。
					// 留着是因为 A2 那套机制在外壳里还完整可用：哪天真出现一个必须拷进内核
					// 的插件，UI 不用重写。所以判据是「有条目才渲染」，而不是「路由通就渲染」——
					// 后者会画出一个空标题，还和下面那个「桌面自带」撞名。
					desktop.available && desktop.plugins.length > 0 ? react_jsx_runtime.jsxs("div", { className: "dsmkGroup", children: [
						react_jsx_runtime.jsx(GroupTitle, { title: t("market.group.desktop"), hint: t("market.group.desktopHint") }),
						pendingCount > 0 ? react_jsx_runtime.jsx("div", { className: "dsmkPending", children: fmt(t("market.desktop.pending"), { n: pendingCount }) }) : null,
						toggleError ? react_jsx_runtime.jsx("div", { className: "dsmkPending dsmkResultErr", children: toggleError }) : null,
						desktop.plugins.map((item) => react_jsx_runtime.jsx(DesktopRow, {
							t, item,
							desired: Object.prototype.hasOwnProperty.call(pending, item.entryId) ? pending[item.entryId] : item.active,
