window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".dsmkCardThumb{position:relative;height:104px;border-radius:8px;overflow:hidden;background:var(--dsw-alias-bg-layer-2,rgba(255,255,255,.06));border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08))}",
			...
			// min-height 让同一排的卡片等高、也让「有图」和「无图」两种卡片一样高；
			// padding-bottom 给下面那条绝对定位的两行页脚（元信息 + 按钮）留出位置。
			".dsmkCardDiscover{min-height:252px;padding-bottom:74px}",
			// 页脚两行：按钮在上，下载量 + 更新日期贴着卡片底边在最下面。挤成一行试过
			// ——卡片只有三分之一面板宽，「周下载 1.2万 · 更新于 2026-08-31」加上两个
			// 按钮必然溢出，元信息被挤成省略号等于没显示。
			//
			// bottom 给 12px 而不是跟着卡片 padding 走的 14px：元信息那行是 11px 的
			// 小字，行盒上下本来就带着一点行距，视觉上的底边比盒子底边高一截，取
			// 14px 看着反而是「浮着的」。
			".dsmkCardFooterSplit{position:absolute;left:14px;right:14px;bottom:12px;margin-top:0;flex-direction:column;align-items:stretch;gap:8px}",
			".dsmkCardFooterActions{display:flex;gap:8px;justify-content:flex-end}",
			// 元信息一行放不下就整体省略，不换行——换行会把卡片顶高，而卡片是等高的。
			".dsmkCardMetaLine{display:flex;align-items:center;gap:6px;min-width:0;font-size:11px;color:var(--dsw-alias-label-tertiary,#8b949e);white-space:nowrap;overflow:hidden}",
		...
		function loadPreview(item, onDone) {
			...
			if (waiting !== undefined) { waiting.push(onDone); return; }
			previewWaiters.set(name, [onDone]);
			previewQueue.push(async () => {
				let src = null;
				// slug 会一起带上：README 里写相对路径的图，服务端要靠它还原成
				// GitHub raw 地址，不给就只能丢掉那些图。
				try { src = await fetchPreviewSrc(item); } catch { /* 取不到就是没有图，卡片照常显示 */ }
				previewCache.set(name, src);
				const waiters = previewWaiters.get(name) ?? [];
		...
		function DiscoverRow({ t, item, expanded, onToggle, installedNames, downloadsLabel, showTagged, action, canInstall, busy, onInstall }) {
			...
			// 缩略图：卡片滚进视野才去要（代价见 loadPreview 的注释）。`undefined` 是
			// 「还没问过」，`null` 是「问过了，没有图」——两者必须分开，否则没有图的
			// 包每次重渲染都会再排一次队。
			const cardRef = react.useRef(null);
			const [thumb, setThumb] = react.useState(() => previewCache.get(item.name) ?? null);
			...
			react.useEffect(() => {
				...
				const observer = new IntersectionObserver((entries) => {
					...
					// 只要一次：图要么有要么没有，不会因为再滚进来一次就变。
					observer.disconnect();
					loadPreview(item, (src) => { if (alive) setThumb(src); });
				}, { rootMargin: "240px" });
			...
			return react_jsx_runtime.jsxs("div", {
				...
				children: [
					...
					item.description ? react_jsx_runtime.jsx("div", {
						className: "dsmkCardDesc" + (showThumb ? "" : " dsmkCardDescTall"),
					...
					// 底部两行：上面是「详情」+「安装」，最底下贴着卡片底边的是这张卡片
					// 的两个数字（下载量、最后发版日期）。日期原来单独占一行、而且只画
					// 一个裸日期（`2026-08-31`）——看不出它说的是「发版」还是「安装」，
					// 现在和下载量并排，并写成「更新于 …」。
					//
					...
					react_jsx_runtime.jsxs("div", { className: "dsmkCardFooter dsmkCardFooterSplit", children: [
						...
						// 元信息压在最底下一行。它是这张卡片上分量最轻的东西（两个旁证
						// 数字），压在按钮下面贴着卡片底边，视线从上往下扫是「这是什么 →
						// 长什么样 → 能拿它怎么办 → 顺带一提它多热门/多新」。
			...
			return react_jsx_runtime.jsxs("div", { className: "dsmkCard dsmkCardStatic" + (removed ? " dsmkCardRemoved" : ""), title: hint, children: [
				...
				item.removable && !removed ? react_jsx_runtime.jsxs("div", { className: "dsmkCardFooter", children: [
					...
					item.updateAvailable && !updateJustFinished ? react_jsx_runtime.jsx("button", {
						...
						onClick: () => onUpdate(item.name, item.latestVersion),
		...
		function MarketPanelBody({ t, open, updates, onClose }) {
			...
			const runInstall = react.useCallback(async (name, version) => {
				...
				// 就可能压根没机会写。
				if (name === SELF_NAME) writeSelfUpdate({ name, version: version ?? "", n: pendingCount + 1 });
				try {
		...
		// 只给单测用。
		exports.__test__ = { fmt, shortDate, CommandBox, DiscoverRow, InstalledRow, ActionResult, fetchPreviewSrc, previewCache };
