window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function InstalledRow({ t, item, action, busy, onUninstall, onToggle, canInstall }) {
		...
		function MarketPanelBody({ t, open, onClose }) {
			...
			react.useEffect(() => {
				...
				const qs = `q=${encodeURIComponent(debounced)}&size=30&sort=${encodeURIComponent(sort)}${onlyDsh ? "" : "&all=1"}`;
