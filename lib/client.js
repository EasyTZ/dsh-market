window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"market.pending.restart": "有插件改动，重启后生效",
		...
		const en = {
			...
			"market.pending.restart": "Plugin changes pending — restart to apply",
		...
		function MarketPanelBody({ t, open, onClose }) {
			...
			// 只加不减：这一轮面板打开期间只要发生过一次，就一直提示到重启为止。
			const [actionRestartNeeded, setActionRestartNeeded] = react.useState(false);
			const restartPending = togglePending > 0 || actionRestartNeeded;
			...
			const installedContent = () => {
				...
				return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsmkGroup", children: [
						...
						// 出现，点一下开关这行突然冒出来，会把下面卡片网格（以及里面的卸载
						// 按钮）一起往下挤一截，手感就是「点完按钮，位置变了」。占位不占交互：
						// 隐藏时按钮本身也不可 tab/点，不会被当成一个「什么都不做」的按钮。
						react_jsx_runtime.jsxs("div", { className: "dsmkPending" + (restartPending ? "" : " dsmkPendingHidden"), children: [
							react_jsx_runtime.jsx("span", { children: togglePending > 0 ? fmt(t("market.toggle.pending"), { n: togglePending }) : actionRestartNeeded ? t("market.pending.restart") : " " }),
							restartAvailable()
								? react_jsx_runtime.jsx("button", {
